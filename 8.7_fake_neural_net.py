...
from scipy import stats                                 # to calculate the entropy
...
class SigmoidFeaturizer:
    ...
    def transform(self, X):
        # (Ntest x D) x (D x Nsamples) -> (Ntest x Nsamples)
...
pipeline = Pipeline([
    ...
    ('sigmoid', SigmoidFeaturizer(gamma=0.05, n_components=1000, method='gmm')),
    # ('linear', SGDClassifier(max_iter=1e6, tol=1e-5))
    ('linear', LogisticRegression(max_iter=1e6))  # takes longer
])
